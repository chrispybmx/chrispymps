...
- [ ] **BLOCCATO — migration `20260819c_spot_type_transition.sql` NON applicata.** `ALTER TYPE spot_type ADD VALUE 'transition'` sul DB di produzione è stato fermato dal classifier di sicurezza. Non ho tentato aggiramenti. Finché non è applicata, la categoria si vede nei filtri ma **assegnarla a uno spot darebbe errore `22P02 invalid input value for enum`**. Va lanciata a mano in SQL Editor
- [ ] Ri-categorizzare gli spot: 60 su 116 sono `street`, e una parte sono transition. Serve l'occhio di chi conosce gli spot, non è automatizzabile
