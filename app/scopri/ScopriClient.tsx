  ...
  return <main className={styles.page}>
    ...
    <div className={styles.results} ref={scroller}>
      <div className={styles.resultsInner}>
        ...
        <div className={styles.resultHeading}><h2 tabIndex={-1} ref={heading}>{loadError ? text('Spot non disponibili', 'Spots unavailable') : `${filtered.length} ${text('spot', filtered.length === 1 ? 'spot' : 'spots')}`}</h2>{!loadError && <span>{filters.sort === 'newest' ? text('In ordine di aggiunta', 'Ordered by date added') : text('In ordine alfabetico', 'Alphabetical order')}</span>}</div>
