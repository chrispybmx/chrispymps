...
/**
 * GET /api/spot-likes?spot_id=X
 * Returns like count + whether current user liked it
 */
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    count: spot?.likes_count ?? 0,
    hasLiked,
  });
}

/**
 * POST /api/spot-likes { spot_id }
 * Toggle like. Returns { ok, hasLiked, count }
 */
export async function POST(req: NextRequest) {
  ...
  if (existing) {
    await sb.from('spot_likes').delete().eq('spot_id', spot_id);
  } else {
    await sb.from('spot_likes').insert({ spot_id });
  }
