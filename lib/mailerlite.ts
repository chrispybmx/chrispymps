 ...
 *
 * Service groups only. Marketing consent uses the private ledger and its worker.
 */
...
export async function subscribeToNewsletter(
  ...
  const groupId =
    opts.groupId ??
    (opts.source === 'submit-spot' && process.env.MAILERLITE_SERVICE_GROUP_ID ? process.env.MAILERLITE_SERVICE_GROUP_ID : opts.source ? GROUP_BY_SOURCE[opts.source] : undefined) ??
