...
export default function ShareSpotBtn({ spotName, spotSlug, city }: Props) {
  ...
  const handleShare = async () => {
    // Mobile → menu nativo (include WhatsApp, Instagram, ecc.)
    if (typeof navigator !== 'undefined' && navigator.share) {
      try { await navigator.share({ title: spotName, text, url }); } catch {}
      return;
    }
    // Desktop → apri mini-menu
    setOpen(o => !o);
  };
  ...
  return (
    <div style={{ padding: '0 20px 28px', position: 'relative' }}>
      ...
      <button
        ...
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'var(--gray-600)'; (e.currentTarget as HTMLElement).style.color = 'var(--gray-300)'; }}
      >
